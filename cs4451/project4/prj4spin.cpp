...
/* For english determination */
enum {TOP, BACK, LEFT, RIGHT};
...
/* function for figuring out if the ball is hit with top, back, left
 * or right spin
 */
void determine_english(float english_x, float english_y, 
		       float pullback_x, float pullback_y)
{
  printf("english= <%f, %f>\n", english_x, english_y);  
  printf("pullback= <%f, %f>\n", pullback_x, pullback_y);  

  if((english_x>0 && english_y>0 && pullback_x>0 && pullback_y>0) ||
     (english_x<=0 && english_y<=0 && pullback_x<=0 && pullback_y<=0)) 
    english = BACK;
...
void mouse(int button, int state, int x, int y)
{
  ...
  if (no_motion) {
	...
	for (i = 0; i < ball_count; i++) {
	  if((startX < (ball_pos[i].X()+ball_r))&& 
		...
		break;
	      ...
	      if(cue_ball != -1){
		/* determines the spin put on the ball */
		english_x = ball_pos[i].X()-startX;
		english_y = ball_pos[i].Y()-startY;
	      }
	    }
	}
      }
	...
	printf("pullback= <%f, %f> \n", pullback_x, pullback_y);
	printf("pullmag= %f \n",
	       sqrt( pow(startX-endX, 2) + pow(startY-endY, 2) ));
	       */
	...
	determine_english(english_x, english_y, pullback_x, pullback_y);
	if (cue_ball != -1) {

	  /*
	  ball_vel[cue_ball] = new_veloc;
	  if(english == TOP) 
	    mass[cue_ball] = mass[cue_ball]*(3 * (sqrt(pow(english_x,2)+
						       pow(english_y,2))));
	  if(english == BACK)
	    mass[cue_ball] = mass[cue_ball]/(3 * (sqrt(pow(english_x,2)+
						       pow(english_y,2))));
